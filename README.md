...
1. 从 [Releases](https://github.com/sixlab/LinkSentinel/releases/latest) 下载对应版本的 `macOS-universal.zip`，支持 Apple Silicon 和 Intel Mac。下文 HEAD 测速行为从 1.0.3 起提供，已发布版本的变化见对应 Release 说明。
...
2. 默认链接为 `https://www.gstatic.com/generate_204`，告警阈值为 **1000 毫秒**，时间间隔为 **5 秒**。
...
7. 停止时点击“重置”，恢复默认链接 `https://www.gstatic.com/generate_204`、1000 毫秒阈值和 5 秒间隔；重启应用后仍使用默认值。监控或准备启动期间按钮禁用，历史记录保留。
...
- 告警阈值不会中断请求。收到响应头后记录响应耗时，超过阈值时显示黄色“延迟高”并通知。HTTP 2xx/3xx（包括默认地址的 204）作为有效响应；HTTP 4xx/5xx、网络、DNS/TLS 错误显示红色“失败”并通知，即使耗时已经超过阈值。不支持 HEAD 的服务器返回 405 时会记录失败，不自动改用 GET。底层连接仍遵循 URLSession 默认网络超时规则，发生传输超时也会记录实际耗时和失败原因。
...
- 升级到 1.0.3 时，仅将已保存的旧默认 Google 首页一次性替换为新的 204 地址；自定义链接、阈值和间隔保留。旧记录的数值不变，原“超时”结果统一显示为“延迟高”。
