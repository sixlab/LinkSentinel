...
定时用 HEAD 请求检测链接，记录收到响应头的耗时，在延迟高或请求失败时发送系统通知。关闭窗口后仍可在菜单栏继续监控。

**这是一个纯 AI 开发的项目。** 人负责提出需求和反馈，AI 完成代码与界面实现、测试编写、文档整理，以及构建和发布流程。

## 下载与安装

1. 从 [Releases](https://github.com/sixlab/LinkSentinel/releases/latest) 下载对应版本的 `macOS-universal.zip`，支持 Apple Silicon 和 Intel Mac。HEAD 测速从 1.0.3 起提供，连续异常通知从 1.0.4 起提供；已发布版本的功能见对应 Release 说明。
...
2. 默认链接为 `https://www.gstatic.com/generate_204`，告警阈值为 **1000 毫秒**，时间间隔为 **5 秒**，连续异常次数为 **3 次**。
3. 点击“开启监控”，允许系统通知，即刻开始第一次请求。
4. **灰色：未开启；绿色：监控中；黄色：延迟高；红色：失败。** 网络、DNS/TLS 和 HTTP 错误显示红色；下次请求正常后恢复绿色。
5. 关闭窗口会隐藏 Dock 图标，保留菜单栏并继续监控。双击菜单栏圆点再次打开窗口时，Dock 图标恢复；右键菜单可打开窗口、停止监控或退出。最小化窗口保留 Dock 图标。
6. 点击“停止监控”取消进行中的请求。停止后可修改配置，重新开启。
7. 停止时点击“重置”，恢复默认链接 `https://www.gstatic.com/generate_204`、1000 毫秒阈值、5 秒间隔和 3 次连续异常；重启应用后仍使用默认值。监控或准备启动期间按钮禁用，历史记录保留。

快捷键：⌘Return 开启/停止，⌘W 关闭窗口，⌘Q 退出。监控时链接仍可选中复制，其余配置需停止后修改。

## 请求与通知规则

- 使用 HTTP HEAD，禁用响应缓存，不跟随重定向；耗时覆盖从发起请求到收到响应头（包括 DNS、连接和 TLS），不下载正文。该请求方式与 [Mihomo 普通 URLTest](https://github.com/MetaCubeX/mihomo/blob/Meta/adapter/adapter.go) 一致，未启用其“统一延迟”的第二次请求机制。实际数值仍会受系统代理路径和网络栈差异影响。
- 告警阈值不会中断请求。收到响应头后记录响应耗时，超过阈值时显示黄色“延迟高”，并计入连续异常。HTTP 2xx/3xx（包括默认地址的 204）作为有效响应；HTTP 4xx/5xx、网络、DNS/TLS 错误显示红色“失败”并计入连续异常，即使耗时已经超过阈值。不支持 HEAD 的服务器返回 405 时会记录失败，不自动改用 GET。底层连接仍遵循 URLSession 默认网络超时规则，发生传输超时也会记录实际耗时和失败原因。
- 阈值支持 1～3600000 的整数毫秒；间隔支持 0.1～86400 秒；连续异常次数须为正整数，默认 3。
- 相邻请求开始时间至少相隔设定的间隔；请求串行执行。如果单次耗时超过间隔，则在本次结束后发起下一次，不并发补发积压请求。
- 延迟高和请求失败混合累计，每连续达到设定次数通知一次。默认在第 3、6、9 次异常时通知，正常请求、取消或停止后清零；设为 1 时每次异常均通知。状态颜色仍立即反映每次请求结果，不必等到发送通知。历史表中显示开始时间、链接、耗时、结果、通知发送状态，鼠标悬停结果可看具体原因。
- “是 · 已发送”表示系统已接受通知；右上角是否显示横幅仍受系统通知样式、专注模式控制。请在 **系统设置 → 通知 → 链接哨兵** 中允许通知并开启横幅。
- 未授权通知时仍可监控，但会显示权限提示，达到通知次数的记录标记“未发送 · 未授权”；未达到次数的记录显示“否”。前台窗口打开时也支持横幅。
...
- 点击历史行即可选中，按住 ⌘ 或 Shift 可多选；右键选择“复制行”（多选时为“复制所选 N 行”），或按 ⌘C。复制内容包含完整日期时间、链接、响应耗时、结果和通知状态，按列表顺序排列，以制表符分列，便于粘贴到表格。
...
- 升级到 1.0.3 时，仅将已保存的旧默认 Google 首页一次性替换为新的 204 地址；自定义链接、阈值和间隔保留。旧记录的数值不变，原“超时”结果统一显示为“延迟高”。升级到 1.0.4 时，旧配置缺少的连续异常次数默认为 3，其余配置保留。
...
自动测试使用可控的 URLProtocol 替代外部 HTTP 传输，覆盖 HEAD 方法、204 响应、响应头计时、不跟随重定向、慢响应和失败判定、手动取消及默认地址迁移；SQLite 使用临时数据库验证持久化和分页。
